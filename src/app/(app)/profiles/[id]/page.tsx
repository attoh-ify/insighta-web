  ...
  return (
    <div>
      <div style={{ marginBottom: "28px" }}>
        <Link href="/profiles" style={{ fontSize: "0.8rem", fontFamily: "DM Mono, monospace", color: "var(--ink-300)", textDecoration: "none", marginBottom: "12px", display: "inline-block" }}>
          ← Back to Profiles
        </Link>
        <h1 style={{ fontSize: "1.6rem", fontWeight: 800, letterSpacing: "-0.02em" }}>{profile.name}</h1>
        <p style={{ color: "var(--ink-300)", fontSize: "0.8rem", fontFamily: "DM Mono, monospace", marginTop: "4px" }}>{profile.id}</p>
      </div>
          ...
          <tbody>
            {fields.map(([label, value, mono], i) => (
              <tr key={label} style={{ borderBottom: i < fields.length - 1 ? "1px solid var(--border)" : "none" }}>
                <td style={{ padding: "14px 20px", width: "180px", fontSize: "0.72rem", fontFamily: "DM Mono, monospace", color: "var(--ink-300)", textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</td>
                <td style={{ padding: "14px 20px", fontFamily: mono ? "DM Mono, monospace" : "Syne, sans-serif", fontSize: "0.875rem", color: "var(--ink-50)", fontWeight: mono ? 400 : 500 }}>{value}</td>
              </tr>
            ))}
          </tbody>
        </table>
